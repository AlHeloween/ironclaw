...
| `test_universal_search.py` | Universal Search Service E2E: builds binary, starts service, creates test indexes, verifies `/local/search`, `/local/symbol-search`, `/local/index`, `/local/index/stats`, and `/health` endpoints return correct results |
