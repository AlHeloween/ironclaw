...
async function setupNuqPostgres(): Promise<Services["nuqPostgres"]> {
  ...
  // Running locally without Docker: skip container management if SKIP_DOCKER_SERVICES
  if (process.env.SKIP_DOCKER_SERVICES === "true") {
    logger.info("SKIP_DOCKER_SERVICES=true, skipping PostgreSQL container management");
    config.NUQ_DATABASE_URL = process.env.NUQ_DATABASE_URL || "postgresql://postgres:postgres@localhost:5432/postgres";
    config.NUQ_DATABASE_URL_LISTEN = process.env.NUQ_DATABASE_URL_LISTEN || config.NUQ_DATABASE_URL;
    process.env.NUQ_DATABASE_URL = config.NUQ_DATABASE_URL;
    process.env.NUQ_DATABASE_URL_LISTEN = config.NUQ_DATABASE_URL_LISTEN;
    return undefined;
  }
...
async function setupNuqRabbitMQ(): Promise<Services["nuqRabbitMQ"]> {
  ...
  // Running locally without Docker: skip RabbitMQ
  if (process.env.SKIP_DOCKER_SERVICES === "true") {
    logger.info("SKIP_DOCKER_SERVICES=true, skipping RabbitMQ container management");
    config.NUQ_RABBITMQ_URL = "";
  ...
  const tasks = [
    ...
    (async () => {
      if (process.env.USE_GO_MARKDOWN_PARSER === "false") {
        logger.info("Skipping Go dependencies (USE_GO_MARKDOWN_PARSER=false)");
        return;
      }
