...
function selectModel(modelName: string, retryName: string) {
  if (config.ANTHROPIC_API_KEY) {
    return {
      model: getModel("claude-sonnet-4-20250514", "anthropic"),
      retryModel: getModel("claude-3-5-haiku-20241022", "anthropic"),
    };
  }
  return {
    model: getModel(modelName, "openai"),
    retryModel: getModel(retryName, "openai"),
  };
}
...
export async function generateCompletions({
  ...
  previousWarning,
  isExtractEndpoint,
  model = config.ANTHROPIC_API_KEY ? getModel("claude-sonnet-4-20250514", "anthropic") : getModel("gpt-4o-mini", "openai"),
  mode = "object",
  providerOptions,
  retryModel = config.ANTHROPIC_API_KEY ? getModel("claude-3-5-haiku-20241022", "anthropic") : getModel("gpt-4.1-mini", "openai"),
  costTrackingOptions,
  metadata,
...
export async function performLLMExtract(
  ...
  if (jsonFormat) {
    ...
    const generationOptions: GenerateCompletionsOptions = {
      ...
      model: selectModel(modelSelection.modelName, "gpt-4.1-mini").model,
      retryModel: selectModel(modelSelection.modelName, "gpt-4.1-mini").retryModel,
      costTrackingOptions: {
...
Return the cleaned markdown content preserving the original markdown formatting.`,
    ...
    model: (() => {
      ...
      return selectModel(selection.modelName, "gpt-4.1-mini").model;
    })(),
    retryModel: selectModel("gpt-4.1-mini", "gpt-4.1-mini").retryModel,
    costTrackingOptions: {
      costTracking: meta.costTracking,
      metadata: {
        module: "scrapeURL",
...
export async function generateSchemaFromPrompt(
  ...
  const { model, retryModel } = selectModel("gpt-4o-mini", "gpt-4.1-mini");
