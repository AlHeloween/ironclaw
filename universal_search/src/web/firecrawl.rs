...
#[derive(Debug, Deserialize)]
#[serde(untagged)]
enum FirecrawlSearchData {
    NewFormat { results: Vec<FirecrawlSearchResult> },
    OldFormat(Vec<FirecrawlSearchResult>),
}
...
pub async fn execute_web_search(
    ...
    while attempt < MAX_RETRIES {
        ...
        if attempt < MAX_RETRIES && (resp.status() == 429 || resp.status().is_server_error()) {
            tracing::warn!("Firecrawl error {} (attempt {}/{}). Retrying...", resp.status(), attempt, MAX_RETRIES);
    ...
    let fc_response: FirecrawlSearchResponse = serde_json::from_str(&body).map_err(|e| format!("Failed to parse Firecrawl response: {}", e))?;

    if let Some(err) = fc_response.error {
        return Err(format!("Firecrawl API error: {}", err));
    }

    let results = match fc_response.data {
        Some(FirecrawlSearchData::NewFormat { results }) => results,
        Some(FirecrawlSearchData::OldFormat(results)) => results,
        None => Vec::new(),
    };

    let formatted: Vec<WebSearchResult> = results.into_iter().filter_map(|r| {
        let url = r.url.clone()?;
