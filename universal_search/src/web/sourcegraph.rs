...
pub async fn execute_sourcegraph(
    ...
    let graphql_query = r#"query($q: String!) { search(query: $q) { results { results { ... on FileMatch { file { path name } repository { name } } } matchCount } } }"#;
    ...
    let sg_response: SourcegraphResponse = serde_json::from_str(&body).map_err(|e| format!("Failed to parse Sourcegraph response: {}", e))?;
    ...
    let search_results = sg_response.data.and_then(|d| d.search).and_then(|s| s.results);
    let match_count = search_results.as_ref().and_then(|r| r.match_count).unwrap_or(0);
    let results = search_results.and_then(|r| r.results).unwrap_or_default();

    let formatted: Vec<SourcegraphResult> = results.into_iter().filter_map(|m| {
        let mut entry = SourcegraphResult {
            repository: None, path: None, file: None, line: m.line, source: "sourcegraph".to_string(),
        };
        if let Some(repo) = &m.repository {
            if let Some(name) = &repo.name { entry.repository = Some(name.clone()); }
        }
        if let Some(file) = &m.file {
            if let Some(path) = &file.path { entry.path = Some(path.clone()); }
            if let Some(name) = &file.name { entry.file = Some(name.clone()); }
        }
        if entry.repository.is_none() && entry.path.is_none() && entry.file.is_none() { return None; }
        Some(entry)
    }).collect();
